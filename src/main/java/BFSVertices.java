        ...
        while(!queue.isEmpty()) {
            ...
            for (UUID neighbour : incident.get(v)) {
                ...
                if (!l.get(neighbour)) {
                    ...
                    String vLabel = graph.getVertices().get(neighbour).getLabel();
                    if (vLabel != null && !(vLabel.isEmpty()) && !(vLabel.equals("null"))) {
                        labels.add(Integer.parseInt(vLabel));
